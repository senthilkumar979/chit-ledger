...
export function ChitDetailHeroProgressPanel({
  ...
  return (
    <div className="flex w-full flex-col gap-4 rounded-2xl border border-border/80 bg-gradient-to-br from-surface/90 via-card to-accent/[0.04] p-5 shadow-inner sm:max-w-[280px] lg:shrink-0">
      <div>
        <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted">Progress</p>
        <p className="mt-1 text-sm font-medium text-primary">
          {paidCount} of {chit.payments.length} paid
        </p>
      </div>
      <div className="flex justify-center py-1">
        <ProgressRing paid={paidCount} total={chit.payments.length} size={120} />
      </div>
      {showWithdrawalCta ? (
        <div className="space-y-2 border-t border-border/60 pt-4">
          <Button
            ...
            disabled={!chit.matured}
            title={!chit.matured ? 'Pay installment 20 in full to mature this chit' : undefined}
      ...
      {paidFully && chit.matured && !chit.withdrawal ? (
        <p className="text-center text-[11px] text-accent">Ready to record payout.</p>
      ) : null}
    </div>
  );
}
